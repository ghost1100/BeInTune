...
export default function MyLearningPanel({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await (await import("@/lib/api")).apiFetch("/api/admin/bookings");
        ...
        const next = list
          .filter((booking) =>
            booking.student_id === selected
              ? true
              : Boolean(booking.student_user_id && user && booking.student_user_id === user.id),
          )
          .sort((a, b) => {
  ...
  async function uploadResources(files: FileList | null) {
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        const payload = await (await import("@/lib/api")).apiFetch("/api/admin/upload", {
          method: "POST",
      ...
      const response = await (await import("@/lib/api")).apiFetch(`/api/admin/learning/${selected}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          title: "Resources",
  ...
  function activeStudents() {
    if (!students.length) {
      return (
        <p className="text-sm text-foreground/70">No student records found for your account yet.</p>
      );
    }
        ...
        {resources.map((resource) => (
          <article key={resource.id} className="rounded border p-3">
            <header className="flex flex-col gap-1">
              <h3 className="text-sm font-semibold">{resource.title || "Resource"}</h3>
              {resource.created_at && (
                <time dateTime={resource.created_at} className="text-xs text-foreground/70">
  ...
  function handleDialogInputChange(e: React.ChangeEvent<HTMLInputElement> | null) {
    if (!e) return;
    ...
    <>
      ...
      <Dialog open={uploadDialogOpen} onOpenChange={setUploadDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Drag and drop files here or click select. Videos must be 3 minutes or shorter.
            </DialogDescription>
          </DialogHeader>
          ...
          {dialogFiles.length > 0 && (
            <div className="mt-4 max-h-40 overflow-auto">
              {dialogFiles.map((f, idx) => (
                <div key={`${f.name}-${idx}`} className="flex items-center justify-between border-b py-2">
                  <div className="text-sm">{f.name}</div>
                  <div className="flex items-center gap-2">
                    <div className="text-xs text-foreground/70">{(f.size / 1024 / 1024).toFixed(2)} MB</div>
                    <Button size="sm" variant="outline" onClick={() => removeDialogFile(idx)}>
                      Remove
                    </Button>
                  </div>
                </div>
          ...
          <DialogFooter className="mt-4">
            <div className="flex items-center gap-2">
              ...
              <Button disabled={!dialogFiles.length} onClick={startDialogUpload}>
                Upload
              </Button>
            </div>
          </DialogFooter>
      ...
      <section className={cn("space-y-6", className)}>
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-foreground/70">Access shared lesson material, upcoming bookings, and practise files.</p>
        </div>
          ...
          <div className="space-y-4">
            {upcoming && (
              <div className="rounded-lg border bg-muted/40 p-4">
                <h3 className="text-sm font-semibold text-foreground/80">Next lesson</h3>
                ...
                {upcoming.metadata && (
                  <pre className="mt-2 rounded bg-background/80 p-2 text-xs text-foreground/70">{JSON.stringify(upcoming.metadata, null, 2)}</pre>
                )}
              </div>
            )}
            ...
            <div className="rounded-lg border bg-card p-4">
              {user && user.role === "admin" && (
                <div className="flex items-center justify-between gap-3">
                  <p className="text-sm text-foreground/70">Upload resources to share with the selected student.</p>
                  <div className="flex items-center gap-2">
                    <input
                      ref={fileRef}
