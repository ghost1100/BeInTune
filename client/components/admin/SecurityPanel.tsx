        ...
        <div className="space-y-2 max-h-[60vh] overflow-auto">
          {userLogs.map(l => (
            <div key={l.id} className="rounded-md border p-2">
              <div className="text-sm text-foreground/70">{new Date(l.created_at).toLocaleString()}</div>
              <div className="font-medium">{l.username || l.email || 'Unknown'}</div>
              <div className="text-sm">{l.action}</div>
              <div className="text-xs text-foreground/60">{JSON.stringify(l.meta)}</div>
            </div>
          ))}
        </div>
      </div>
