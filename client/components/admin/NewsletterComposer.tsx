...
export default function NewsletterComposer({ onClose }:{ onClose?:()=>void }){
  ...
  const send = async ()=>{
    ...
    try{
      ...
      if(selectedImage) form.append('unsplash_image', selectedImage.full || selectedImage.thumb);
      ...
      setSubject(''); setHtml(''); setFiles([]); setSelectedImage(null); setResults([]);
      onClose?.();
    }catch(err:any){
      console.error(err);
      ...
      <div className="mt-3 grid gap-2">
        ...
        <div className="grid gap-2">
          <div className="flex gap-2">
            <input value={searchQuery} onChange={(e)=>setSearchQuery(e.target.value)} placeholder="Search for header image (Unsplash)" className="h-10 rounded-md border px-3 flex-1" />
            <Button onClick={runSearch} variant="outline">Search</Button>
          </div>

          {selectedImage ? (
            <div className="p-2 border rounded-md">
              <div className="mb-2 text-sm text-foreground/70">Selected image (Unsplash)</div>
              <img src={selectedImage.full || selectedImage.thumb} alt={selectedImage.alt || ''} className="w-full h-48 object-cover rounded" />
              <div className="flex gap-2 mt-2">
                <Button variant="ghost" onClick={()=>setSelectedImage(null)}>Remove</Button>
              </div>
            </div>
          ) : (
            results.length > 0 && (
              <div className="grid grid-cols-3 gap-2">
                {results.map(r => (
                  <button key={r.id} onClick={()=>pickImage(r)} className="rounded overflow-hidden border">
                    <img src={r.thumb} alt={r.alt || ''} className="w-full h-24 object-cover" />
                  </button>
                ))}
              </div>
            )
        ...
        <div className="flex items-center gap-2">
          ...
          <Button onClick={()=>{ setSubject(''); setHtml(''); setFiles([]); setSelectedImage(null); setResults([]); }} variant="outline">Clear</Button>
