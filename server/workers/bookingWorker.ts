...
export function startBookingWorker() {
  const worker = new Worker(
    ...
    async (job: any) => {
      try {
        ...
        if (!bookingId) throw new Error('Missing bookingId in job');

        // Load booking + related slot/student/user info
        const infoQ = await query(
          `SELECT b.id, b.lesson_type, b.guest_name, b.guest_email, b.guest_phone, b.calendar_event_id, b.recurrence_id, s.user_id as student_user_id, u.email as user_email, u.name as user_name, s.instruments as student_instruments, u.phone as user_phone, s.phone as student_phone, sl.id as slot_id, sl.slot_time as time, sl.slot_date as date, sl.teacher_id, sl.duration_minutes
           FROM bookings b
        ...
        // Decrypt guest fields if necessary
        try {
        ...
        // Create calendar event and expand recurrences where applicable
        try {
          const { createCalendarEvent, listInstances, deleteCalendarEvent } = await import('../lib/calendar');
          ...
          // Try to reuse existing event for the slot if present
          let ev: any = null;
          try {
          ...
          if (!ev) {
            ev = await createCalendarEvent({
              ...
              description: `Booking for ${info.guest_name || info.user_name || ''}`,
              startDateTime: startIso,
              endDateTime: endIso,
              recurrence: info.recurrence ? [String(info.recurrence)] : undefined,
            });
          }
          ...
          if (ev && ev.id) {
            ...
            try {
              await query('UPDATE bookings SET calendar_event_id = $1, recurrence_id = $2 WHERE id = $3', [ev.id, recurrenceIdToStore, bookingId]);
            } catch (e) {
              console.warn('Failed to persist calendar_event_id on booking', e);
            }

            // Map instance to booking
            try {
              const startIsoInstance = `${slotDateRaw}T${slotTimeRaw}:00`;
              const endDate = new Date(startIsoInstance);
              ...
              for (const it of instances) {
                ...
                if (!s) continue;
                if (s.startsWith(startIsoInstance.slice(0, 19))) {
                  found = it;
                  break;
                }
                try {
                  const si = new Date(s).toISOString();
                  if (si === startIsoInstance) {
                    found = it;
                    break;
                  }
                } catch (e) {}
            ...
            // Expand recurrence into DB rows (COUNT/UNTIL or fallback)
            try {
              ...
              if (recurrence && typeof recurrence === 'string') {
                ...
                if (count && count > 1) {
                  for (let i = 1; i < count; i++) {
                    try {
                      const dt = new Date(slotDateRaw + 'T' + (slotTimeRaw || '00:00') + ':00');
                      ...
                      try {
                        ...
                        if (existingSlot && existingSlot.rows && existingSlot.rows[0]) newSlotId = existingSlot.rows[0].id;
                          ...
                          const insSlot = await query('INSERT INTO slots(teacher_id, slot_date, slot_time, duration_minutes, is_available) VALUES ($1,$2,$3,$4,true) RETURNING id', [info.teacher_id || null, slotDate, slotTime, duration || 30]);
                      ...
                      if (newSlotId) {
                        try {
                          ...
                          if (!(existingBk && existingBk.rows && existingBk.rows[0])) {
                            ...
                            } catch (e) { console.warn('Failed to insert recurring booking', e); }
                          }
                        ...
                        try {
                          ...
                          if (newSlotId) {
                            try {
                              ...
                              if (!(existingBk && existingBk.rows && existingBk.rows[0])) {
                                const insBk = await query('INSERT INTO bookings(student_id, slot_id, lesson_type, guest_name, guest_email, guest_phone, calendar_event_id, recurrence_id) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id',[info.student_user_id || null, newSlotId, info.lesson_type || null, info.guest_name, info.guest_email, info.guest_phone, ev.id, ev.id]);
                                try { const newBkId = insBk && insBk.rows && insBk.rows[0] && insBk.rows[0].id; if (newBkId) { try { const startIso = `${slotDate}T${slotTime}:00`; const endDt = new Date(startIso); endDt.setSeconds(endDt.getSeconds()+1); const instances2 = await listInstances(ev.id, startIso, endDt.toISOString()); let found2 = null; for (const it of instances2) { const s = it.start && (it.start.dateTime || it.start.date); if (!s) continue; if (s.startsWith(startIso.slice(0,19))) { found2 = it; break;} try { const si = new Date(s).toISOString(); if (si === startIso) { found2 = it; break; } } catch (e) {} } if (found2 && found2.id) await query('UPDATE bookings SET calendar_instance_id = $1 WHERE id = $2',[found2.id, newBkId]); } catch (e) { console.warn('Failed to map instance for new booking', e); } } } catch (e) { console.warn('Failed to insert recurring booking', e); } }
            ...
            } catch (inner) { console.warn('Failed to expand recurring bookings into DB rows:', inner); }
          }
        } catch (err) {
        ...
        console.error('Booking worker failed for job', job && job.id, err);
        throw err;
      }
    },
    { connection: redisUrl ? { url: redisUrl } : connection },
  );
  ...
  worker.on('failed', (job, err) => {
    console.error('Booking job failed', job && job.id, err);
  });
