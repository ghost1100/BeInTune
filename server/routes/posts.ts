...
router.post("/posts", async (req, res) => {
  try {
    ...
    const insert = await query(
      ...
      [author_id || null, null, body || null, true, JSON.stringify({ author_name: author_name || null })]
    );
    const postId = insert.rows[0].id;

    if (attachments && attachments.length) {
      for (const m of attachments) {
        await query("INSERT INTO post_media(post_id, media_id) VALUES ($1,$2) ON CONFLICT DO NOTHING", [postId, m]);
      }
    }
